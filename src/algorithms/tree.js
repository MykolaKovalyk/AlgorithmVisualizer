...
class AvlTree {
    ...
    rotateLeft(node) {
        ...
        let node_parent = node.parent;  // can be non-existent, when parent is root
        let node_right = node.right;    // always exists, because of balance
        let node_right_left = node_right.left; // can be non-existent, when node_right is leaf, and when it's just null

        // should also consider that parent can have target as left or right

        // change left or right for parent of target
        if (node_parent) {
    ...
    rotateRight(node) {
        let node_parent = node.parent;  // can be non-existent, when parent is root
        let node_left = node.left;    // always exists, because of balance
        let node_left_right = node_left.right; // can be non-existent, when node_right is leaf, and when it's just null
    ...
    #prune(node) {
        let node_child = node.left ? node.left : node.right;    // node can have no children

        if (node_child)
            node_child.parent = node.parent;                        // child should now take place of the pruned node
...
export const avlFind = (key) => {
    createTreeIfNotPresent();

    try {
        currentTree.find(key);
    } catch (e) {
    } finally {
        return currentTree.logger.readAll();
    }
}

export const avlInsert = (key) => {
    createTreeIfNotPresent();
...
export const avlClear = () => { currentTree = null; }